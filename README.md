...
This demo recreates the same technique at toy scale (~30 examples instead of 16M)
...
# Step 1: Collect training data from Claude (~$0.50, takes ~2 min)
export ANTHROPIC_API_KEY=your-key-here
python 01_collect_teacher_data.py

# Step 2: Fine-tune a small model on Claude's outputs (~10-30 min on GPU)
...
- **Step 2**: A GPU with 8+ GB VRAM is ideal. Apple Silicon Macs work too.
  CPU-only is possible but slow (~2-3 hours for a 0.5B model).
...
1. **Pre-run Steps 1 and 2** before recording. Step 1 costs ~$0.50 in API
   calls. Step 2 takes 10-30 minutes depending on your GPU.
...
- Run it live (or show a pre-recorded run). The cost estimate at the end
  is the punchline: "our demo cost fifty cents. Scale that up to 16
...
- "The $0.50 Distillation 'Attack' Anthropic Is Warning About"
...
**Swap the student model** — Edit `STUDENT_MODEL` in `02_finetune_student.py`:
...
**Add more prompts** — The more training examples, the better the distilled
model will be. 100-200 well-crafted prompts would show a dramatic improvement.
At 16 million, you'd see why Anthropic is concerned.
